...
class LLMManager:
    def __init__(self, config):
        ...
        self.groq_limiter = RateLimiter(60) # Groq has high limits, but safe default
        
        self.groq_client = AsyncGroq(api_key=config.GROQ_API_KEY)
        
        self.total_tokens_used = {"openai": 0, "gemini": 0, "groq": 0}
        self.total_cost = {"openai": 0.0, "gemini": 0.0, "groq": 0.0}
    ...
    def estimate_cost(self, tokens: int, model: str, is_completion: bool = False) -> float:
        ...
        costs = {
            ...
            "mixtral-8x7b-32768": {"prompt": 0.0, "completion": 0.0}, # Free tier
        }
        
        model_key = "gpt-4" if "gpt-4" in model else "gpt-3.5-turbo" if "gpt" in model else "gemini-pro" if "gemini" in model else "llama-3.3-70b-versatile"
    ...
    async def call_groq(
        ...
        **kwargs
        ...
        try:
            ...
            usage = response.usage
            self.total_tokens_used["groq"] += usage.total_tokens
            # Groq is currently free for these models, but we track usage
    ...
    async def generate(
        ...
        **kwargs
        ...
        if provider == "openai":
            ...
            return await self.call_groq(messages, model, **kwargs)
        else:
